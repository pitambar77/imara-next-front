...
const TanzaniatravelguidelandingForm = ({ editData, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editData) {
        res = await API.put(
          `/tanzaniatravelguidelanding/${editData._id}`,
          data,
        );
      } else {
        res = await API.post("/tanzaniatravelguidelanding", data);
      }

      alert("Saved Successfully!");
       router.push("/dashboard/tanzania-travelguide-landing");
  ...
  return (
    <form className="grid grid-cols-2 gap-4 pb-20" onSubmit={handleSubmit}>
      <h2 className="text-2xl font-bold col-span-2">
        {editData ? "Edit tanzania Landing" : "Create tanznia Landing"}
      </h2>
      ...
      >
        {editData ? "Update Tanzania travelguide Landing" : "Save Tanzania travelguide Landing"}
      </button>
    </form>
  );
};

export default TanzaniatravelguidelandingForm;
