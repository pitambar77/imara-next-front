          ...
          {s.type === "paragraph" && (
            <div>
                ...
                <div key={i} className="flex gap-2 mb-2">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      updateSection(s.id, { content: updated });
                    }}
                  ...
                  <button
                    onClick={() => {
                      const updated = s.content.filter((_, idx) => idx !== i);
                      updateSection(s.id, { content: updated });
                    }}
              ...
              <button
                onClick={() =>
                  updateSection(s.id, {
                    content: [...s.content, { type: "text", value: "" }],
                  })
                }
        ...
        <>
          {s.faqs.map((faq, i) => (
            <div key={i} className="border p-4 mb-4 rounded-xl">
              ...
              {/* ANSWER BLOCKS */}
              {faq.answerBlocks.map((block, j) => (
                <div key={j} className="mb-3 p-3 border rounded">
                  {/* TYPE */}
                  <select
                    value={block.type}
                    onChange={(e) => {
                      const updated = [...s.faqs];
                      updated[i].answerBlocks[j].type = e.target.value;
                      updateSection(s.id, { faqs: updated });
                    }}
                  ...
                  {block.type === "paragraph" && (
                    <textarea
                      ...
                      onChange={(e) => {
                        const updated = [...s.faqs];
                        updated[i].answerBlocks[j].text = e.target.value;
                        updateSection(s.id, { faqs: updated });
                      }}
                      className="w-full border p-2"
                    />
                  ...
                  {block.type === "list" && (
                    <>
                      {(block.items || []).map((item, k) => (
                        <div key={k} className="flex gap-2 mb-2">
                          <input
                            ...
                            onChange={(e) => {
                              const updated = [...s.faqs];
                              updated[i].answerBlocks[j].items[k] =
                                e.target.value;
                              updateSection(s.id, { faqs: updated });
                            }}
                            className="border p-2 flex-1"
                          ...
                          <button
                            onClick={() => {
                              const updated = [...s.faqs];
                              updated[i].answerBlocks[j].items = updated[
                                i
                              ].answerBlocks[j].items.filter(
                                (_, idx) => idx !== k,
                              );
                              updateSection(s.id, { faqs: updated });
                            }}
                      ...
                      <button
                        onClick={() => {
                          const updated = [...s.faqs];
                          updated[i].answerBlocks[j].items = [
                            ...(updated[i].answerBlocks[j].items || []),
                            "",
                          ];
                          updateSection(s.id, { faqs: updated });
                  ...
                  <button
                    onClick={() => {
                      const updated = [...s.faqs];
                      updated[i].answerBlocks = updated[i].answerBlocks.filter(
                        (_, idx) => idx !== j,
                      );
                      updateSection(s.id, { faqs: updated });
                    }}
...
export default function BlogCreateForm({ id }) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
            ...
            switch (section.type) {
              ...
              case "paragraph":
                ...
                if (Array.isArray(section.content)) {
                  // 🔥 FIX: flatten if nested array
                  if (Array.isArray(section.content[0])) {
                    parsedContent = section.content.flat();
                  } else {
                  ...
                  try {
                    ...
                    console.error("Invalid paragraph JSON:", section.content);
                  }
                }
        ...
        console.log("PARAGRAPH RAW:", res.data.sections);
  ...
  //   axios
  //     .get(`${API_BASE}/imarablog/${id}`)
  //     .then((res) => {
  //       const b = res.data; // 🔥 important
  ...
  async function handleAuthorUpload(file) {
    return enqueueUpload(async () => {
      const form = new FormData();
      form.append("file", file);
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (id) {
        await axios.put(`${API_BASE}/imarablog/${id}`, form, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        setSuccess("Blog updated ✓");
      } else {
        await axios.post(`${API_BASE}/imarablog`, form, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {/* AUTHOR SECTION */}
        <div className="mt-8 border p-4 rounded-xl bg-white">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              handleAuthorUpload(file);
            }}
            className="w-full border p-2 mb-2"
          />

          {/* PREVIEW */}
          {(author.image?.preview || author.image?.url) && (
            <img
              src={
                author.image.preview
                  ? author.image.preview
                  : `${API_BASE}${author.image.url}`
              }
          ...
          {/* ROLE */}
          <input
            placeholder="Role (Owner, Founder...)"
            value={author.role || ""}
          ...
          <input
            ...
            value={author.social?.facebook || ""}
            onChange={(e) =>
              setAuthor({
                ...author,
                social: { ...author.social, facebook: e.target.value },
              })
            }
            className="w-full border p-2 mb-2"
          />
          ...
          <input
            ...
            onChange={(e) =>
              setAuthor({
                ...author,
                social: { ...author.social, instagram: e.target.value },
              })
            }
            className="w-full border p-2 mb-2"
          />
          ...
          <input
            ...
            onChange={(e) =>
              setAuthor({
                ...author,
                social: { ...author.social, twitter: e.target.value },
              })
            }
            className="w-full border p-2 mb-2"
          />
          ...
          <input
            ...
            onChange={(e) =>
              setAuthor({
                ...author,
                social: { ...author.social, youtube: e.target.value },
              })
            }
            className="w-full border p-2"
          />
          ...
          if (s.type === "paragraph") {
            return (
              <p key={s.id}>
                {s.content.map((block, i) => {
                  if (block.type === "bold")
                    return <strong key={i}>{block.value}</strong>;
                  ...
                  <div key={i} className="mb-4">
                    ...
                    {faq.answerBlocks.map((block, j) => {
                      ...
                      if (block.type === "heading")
                        return <h4 key={j}>{block.text}</h4>;

                      if (block.type === "list")
                        return (
                          <ul key={j} className="list-disc ml-6">
