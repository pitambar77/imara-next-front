...
export async function generateMetadata({ params }) {
  ...
  return {
    title: seo?.metaTitle || matched.title,
    description: seo?.metaDescription || matched.subtitle,
    keywords: seo?.keywords || `Tanzania safari, ${matched.title}`,
    alternates: {
    ...
    openGraph: {
      title: seo?.metaTitle || matched.title,
      description: seo?.metaDescription || matched.subtitle,
      images: [seo?.ogImage || matched.image],
      url:
        seo?.canonicalUrl ||
        `https://imarakilelenisafaris.com/tanzania-destinations/${slug}`,
    },
  };
}
